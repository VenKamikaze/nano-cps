...
import org.awiki.kamikaze.nanocps.db.Store;
import org.awiki.kamikaze.nanocps.message.ActionMessage;
...
public class NanoWebsocketClient
{
  ...
  private List<NanoActionEndpoint> endpoints = new ArrayList<NanoActionEndpoint>();

  public NanoWebsocketClient(String uri, Store store)
  {
    this.uri = uri;
    this.store = store;
  };

  public void subscribe(SubscribeTopicMessage subTopic) throws NanoClientException
  {
  ...
  private void sendMessage(ActionMessage message)
  {
    try
    {
      ...
      NanoActionEndpoint ep = new NanoActionEndpoint(message, store);
      container.connectToServer(ep, URI.create(uri));
    ...
    {
      ex.printStackTrace();
      throw new NanoClientException("Unable to send message: " + message, ex);
    }
  }
  
  public void unsubscribe(UnsubscribeTopicMessage unsubTopic) throws NanoClientException
  {
    sendMessage(unsubTopic);
  }
}
